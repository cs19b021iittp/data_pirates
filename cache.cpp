...
// --------------------- Start of CACHE (Phase-3) varibales ----------------------------
...
// int Num_Tag_Bits_L1 , Num_Tag_Bits_L2 ; 
// we can find their length but we dont need them
...
//-------------------- End of Cache Variables and functions ------------------------
...
int main()
{
   ...
   //-------------------- take inputs for CACHE
    ...
    for ( int i=0; i < Num_SetsInCache_L1 ; i++ )
    {
        for ( int j=0; j < Num_BlocksInSet_L1 ; j++ )
        {
            L1_Cache.sets[i].blocks[j].tag = -1;
            L1_Cache.sets[i].blocks[j].lru = 0; 
        }
        
    }
    for ( int i=0; i < Num_SetsInCache_L2 ; i++ )
    {
        for ( int j=0; j < Num_BlocksInSet_L2 ; j++ )
        {
            L2_Cache.sets[i].blocks[j].tag = -1;
